...
public class XPacker {
	...
	public XPacker(final Event event) {
	...
	public byte[] pack(final byte[] unpackedData) {
		...
		// System.out.println("Unpacked event data:" + unpackedData.length + ':'
		// + DataConverters.getByteArrayHexString(unpackedData, 0,
		// unpackedData.length));
	...
	private void writeParameterValues(final EventParameter parameter) {
		...
		if (parameter.isVariableLength()) {
			if (numberOfBits == -1) {
				...
				parameterLength = (0x0000ff00 & unpackedData[unpackedOffset - 2] << 8) + (0x000000ff & unpackedData[unpackedOffset - 1]);
				numberOfBits = parameterLength * 8;
			} else {
				setParamBytelenght(parameterLength, packedOffset, 0);
				packedOffset += (8 - (unpackedOffset % 8)) & 0x0000007;
			}
		}
